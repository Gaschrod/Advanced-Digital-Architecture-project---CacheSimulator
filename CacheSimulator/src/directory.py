...
class Directory:
    ...
    def process_read(self, core_id, address):
        ...
        self.logger.info(f'\t[Directory] Read request for {address} from Core {core_id}')
        self.logger.info(f'\t[Directory] Current state: {entry.state}, Sharers: {entry.sharers}, Owner: {entry.owner}')

        if entry.state == 'I':
            # No cache has it, fetch from memory
            entry.state = 'S'
            entry.sharers.add(core_id)
            self.logger.info(f'\t[Directory] State I → S, Core {core_id} added to sharers')
            return {'type': 'fetch_from_memory'}

        elif entry.state == 'S':
            # Others have it in Shared, add this core as sharer
            entry.sharers.add(core_id)
            self.logger.info(f'\t[Directory] State S → S, Core {core_id} added to sharers')
            ...
            entry.owner = None
            self.logger.info(f'\t[Directory] State M → S, intervention from Core {owner}, both cores now share')
    ...
    def process_write(self, core_id, address):
        ...
        self.logger.info(f'\t[Directory] Write request for {address} from Core {core_id}')
        self.logger.info(f'\t[Directory] Current state: {entry.state}, Sharers: {entry.sharers}, Owner: {entry.owner}')
        ...
        if entry.state == 'I':
            ...
            action['type'] = 'fetch_from_memory'
            self.logger.info(f'\t[Directory] State S → M, invalidating {entry.sharers}, Core {core_id} becomes owner')

        elif entry.state == 'M':
            # Owner has it modified
            action['type'] = 'fetch_from_owner'
            action['owner'] = entry.owner
            self.logger.info(f'\t[Directory] State M → M, intervention from Core {entry.owner}, Core {core_id} becomes new owner')
    ...
    def process_upgrade(self, core_id, address):
        ...
        self.logger.info(f'\t[Directory] Upgrade request for {address} from Core {core_id}')

        # Core must be in sharers
        if core_id not in entry.sharers:
            self.logger.error(f'\t[Directory] ERROR: Core {core_id} not in sharers for {address}!')
            return {'sharers': set()}

        if entry.state != 'S':
            self.logger.error(f'\t[Directory] ERROR: Upgrade request but state is {entry.state}, not S!')
            return {'sharers': set()}
        ...
        self.logger.info(f'\t[Directory] State S → M, invalidating cores {other_sharers}, Core {core_id} becomes owner')
    ...
    def process_eviction(self, core_id, address, is_dirty):
        ...
        self.logger.info(f'\t[Directory] Eviction of {address} from Core {core_id} (dirty={is_dirty})')

        if entry.state == 'M' and entry.owner == core_id:
            # Evicting modified data
            if is_dirty:
                self.logger.info(f'\t[Directory] Core {core_id} writing back modified {address}')
            entry.state = 'I'
            entry.owner = None
            ...
            if len(entry.sharers) == 0:
                ...
                self.logger.info(f'\t[Directory] State S → S (Core {core_id} removed, remaining: {entry.sharers})')
