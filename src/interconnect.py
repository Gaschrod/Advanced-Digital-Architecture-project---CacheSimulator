...
class Interconnect:
    """Point-to-point messaging layer between processor cores, the directory, and memory.

    Every message travels directly to a single named target (the directory, or
    one specific cache). Coherence decisions are made exclusively by the
    directory using its per-block sharer set and owner field.

    Responsibilities:
    1. Forward processor requests (GetS, GetM, Upgrade) to the directory.
    2. Fetch data from the shared cache / memory when needed and install blocks
       in the requesting core's L1 cache.

    All coherence actions — which caches to contact and what messages to send
    (Fetch, FetchInv, Inv) — are driven by the directory, not this layer.
    """
    ...
    def __init__(self, directory, shared_cache, logger):
        ...
        self.logger = logger
        self.transaction_time = 5  # Interconnect transaction overhead in cycles
        self.cache_to_cache_time = (
            10  # Extra latency when data comes from another cache
        )

    def register_core(self, core):
        """Register a core with the interconnect and the directory."""
        self.cores[core.core_id] = core
        self.directory.register_core(core)
        self.logger.info(f"[Interconnect] Registered Core {core.core_id}")

    def coherent_read(self, core_id, address, current_step):
        """Forward GetS to the directory (I --PrRd--> S).
        ...
        """
        self.logger.info(f"\t[Interconnect] Forwarding GetS from Core {core_id} for {address}")
    ...
    def coherent_write(self, core_id, address, current_step):
        """Forward GetM to the directory (I --PrWr--> M).
        ...
        Returns: Response object with timing information
        """
        self.logger.info(f"\t[Interconnect] Forwarding GetM from Core {core_id} for {address}")
    ...
    def coherent_upgrade(self, core_id, address, current_step):
        """Forward Upgrade to the directory (S --PrWr--> M).
        ...
        """
        self.logger.info(
            f"\t[Interconnect] Forwarding Upgrade from Core {core_id} for {address}"
        )
